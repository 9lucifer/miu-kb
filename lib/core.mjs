...
function demoteMergeCandidate(db, before, reason) {
  db.prepare(`
    UPDATE candidates
    SET memory_action = 'create_new',
        target_kind = NULL,
        target_id = NULL,
        target_status = NULL,
        target_content = NULL,
  ...
  db.prepare(`
    ...
    VALUES (?, ?, 'merge_target_demoted', ?, ?)
  `).run(nowId("evt"), before.id, safeJson(before), safeJson({ ...after, reason }));
  return after;
}

export function demoteMergeDependents(db, targetId, reason = "目标候选已不可合并，自动改为新建记忆。") {
  if (!targetId) return 0;
  const rows = db.prepare(`
    SELECT *
    FROM candidates
    WHERE status IN ('pending', 'ai_reviewing')
      AND memory_action = 'merge_pending'
      AND target_kind = 'review_candidate'
      AND target_id = ?
  `).all(targetId);
  for (const row of rows) demoteMergeCandidate(db, row, reason);
  return rows.length;
}

export function repairInvalidMergeCandidates(db) {
  const rows = db.prepare(`
    SELECT source.*
    FROM candidates source
    LEFT JOIN candidates target ON target.id = source.target_id
    WHERE source.status IN ('pending', 'ai_reviewing')
      AND source.memory_action = 'merge_pending'
      AND source.target_kind = 'review_candidate'
      AND (target.id IS NULL OR target.status IN ('rejected', 'archived', 'deleted'))
  `).all();
  for (const row of rows) demoteMergeCandidate(db, row, "目标候选已不可用，自动改为新建记忆。");
  return rows.length;
}
